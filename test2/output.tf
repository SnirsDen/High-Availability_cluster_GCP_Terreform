...
output "prod_vm_urls" {
  ...
  value       = [for vm in google_compute_instance.prod_vms : "http://${vm.network_interface[0].access_config[0].nat_ip}"]
}

output "dev_vm_urls" {
  description = "URL для доступа к DEV виртуальным машинам"
  value       = [for vm in google_compute_instance.dev_vms : "http://${vm.network_interface[0].access_config[0].nat_ip}"]
}

output "node_exporter_url" {
  description = "URL для доступа к Node Exporter на PROD виртуальных машинах"
  value       = [for vm in google_compute_instance.prod_vms : "http://${vm.network_interface[0].access_config[0].nat_ip}:9100"]
}

output "prod_vm_ips" {
  description = "IP-адреса PROD виртуальных машин"
  value       = google_compute_instance.prod_vms[*].network_interface[0].access_config[0].nat_ip
}

output "dev_vm_ips" {
  description = "IP-адреса DEV виртуальных машин"
  value       = google_compute_instance.dev_vms[*].network_interface[0].access_config[0].nat_ip
}
...
output "prod_vm_zones" {
  ...
  value       = google_compute_instance.prod_vms[*].zone
}

output "dev_vm_zones" {
  description = "Зоны размещения DEV виртуальных машин"
  value       = google_compute_instance.dev_vms[*].zone
}